...
int main()
{
	...
	while (1)
	{
		//减少戴帽程序重复运算量，只有在更换帽子与处理图片时才会进入戴帽程序
		if (flag)
		{
			//选择帽子
			switch (hat_count)
		...
		case(87):case(119):
			// 提醒用户是否保存图像
			result = MessageBox(hwnd, TEXT("是否要保存图像"), TEXT("保存提示框"), MB_ICONINFORMATION | MB_YESNO);
			switch (result)
			{
				//选择yes，保存图像
			case(IDYES):
...
//输出图像初始化
//功能：获取用户输入图像储存地址，若地址下有存储图像，则返回图像矩阵头指针，反之提示用户检查输入图像路径
IplImage* Input_Image_Init()
{
	//stop:
	flag = 1;
...
//人脸检测
//功能：检测单人脸，若成功则返回人脸区域左上角顶点坐标与人脸区域的长与高
void detect_and_draw(IplImage* img)
{
	//调整系数
	double scale = 1.3;
...
//戴帽
//功能：根据用户传递的原图，帽子图与人脸数据结构体对头部进行戴帽
void Tohead(IplImage* img, IplImage* hat)
{
	IplImage* dst;
	//定义ROI
...
void TosaveImage(IplImage* img)
{
	...
	p[1] = 90;
	//存储图像
	save_flag=cvSaveImage(ss, img, p);
	while (!save_flag);
	outText();
	//存储图像成功提示框
